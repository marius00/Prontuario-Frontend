    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2 mb-4">
        <div className="relative flex-1">
          ...
          <Input 
            placeholder={searchType === 'doc' ? "Search by Doc ID or Title..." : "Search by Patient Name..."}
            value={query}
          ...
          {filteredDocs.map(doc => (
            <div key={doc.id} className="relative">
              ...
              <div className="mt-2 ml-4 pl-4 border-l-2 border-dashed border-muted-foreground/20">
                <div className="text-xs font-semibold text-muted-foreground mb-1 uppercase tracking-wider">History</div>
                {getDocumentHistory(doc.id).slice(0, 3).map(event => (
                   <div key={event.id} className="text-xs text-muted-foreground mb-1">
                     <span className="font-mono">{format(new Date(event.timestamp), 'MM/dd HH:mm')}</span> - {event.type} ({event.sectorId})
                   </div>
                ))}
              </div>
            </div>
              ...
              <div key={patient.id} className="border rounded-lg p-4 bg-card space-y-3">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">DOB: {patient.birthdate}</p>
                  </div>
                ...
                {patientDocs.length > 0 && (
                  <div className="space-y-2 pt-2 border-t">
                    <div className="text-xs font-medium text-muted-foreground">Associated Documents:</div>
                    {patientDocs.map(doc => (
